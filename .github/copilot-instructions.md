...
C# .NET 10 CLI agent harness distributed as a dotnet tool: agent loop, tool registry, and provider-agnostic LLM access via Microsoft.Extensions.AI IChatClient.

> Domain: A minimal LLM agent harness — the software layer that turns a language model into a working agent: orchestration loop, tool execution, context management, and state.
> Stacks: dotnet, github, ts, node
> Scaffolded by ai-badger 0.143.0. Source of truth for this file: `.ai-badger/copilot-instructions.md`.
...
- C#/.NET implementation, CLI, tool packaging → `dotnet-engineer`
